...
func set_year(new_year):
	...
	if not validation_failed:
		if not _cast_date():
			...
			# should be fine though. It was not fine... added validation_failed to avoid casting previous date
			selected_date = previous_date.duplicate()
...
func set_month(new_month):
	...
	if _cast_date_value_to_int("month"):
		...
		# this way avoid unnecessary cast_date if already failed
...
func back_one_month(): #attempt to return same day next previous month
	if selected_date["month"] > 1:
		selected_date["month"] -= 1
	else:
		selected_date["year"] -= 1
